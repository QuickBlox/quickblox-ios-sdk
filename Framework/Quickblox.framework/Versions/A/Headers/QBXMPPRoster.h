...
**/
@interface QBXMPPRoster : QBXMPPModule
{
 ...
 */
	id <QBXMPPRosterStorage> xmppRosterStorage;
...
- (id)initWithRosterStorage:(id <QBXMPPRosterStorage>)storage;
- (id)initWithRosterStorage:(id <QBXMPPRosterStorage>)storage dispatchQueue:(dispatch_queue_t)queue;
...
@property (readonly) id <QBXMPPRosterStorage> xmppRosterStorage;
...
**/
- (void)addUser:(QBXMPPJID *)jid withNickname:(NSString *)optionalName;

/**
 * Sets/modifies the nickname for the given user.
**/
- (void)setNickname:(NSString *)nickname forUser:(QBXMPPJID *)jid;
 ...
 * @see revokePresencePermissionFromUser:
**/
- (void)unsubscribePresenceFromUser:(QBXMPPJID *)jid;
 ...
 * @see unsubscribePresenceFromUser:
**/
- (void)revokePresencePermissionFromUser:(QBXMPPJID *)jid;
...
**/
- (void)acceptPresenceSubscriptionRequestFrom:(QBXMPPJID *)jid andAddToRoster:(BOOL)flag;
...
**/
- (void)rejectPresenceSubscriptionRequestFrom:(QBXMPPJID *)jid;
...
@protocol QBXMPPRosterStorage <NSObject>
@required
...
**/
- (BOOL)configureWithParent:(QBXMPPRoster *)aParent queue:(dispatch_queue_t)queue;

- (void)beginRosterPopulationForXMPPStream:(QBXMPPStream *)stream;
- (void)endRosterPopulationForXMPPStream:(QBXMPPStream *)stream;

- (void)handleRosterItem:(NSXMLElement *)item xmppStream:(QBXMPPStream *)stream;
- (void)handlePresence:(QBXMPPPresence *)presence xmppStream:(QBXMPPStream *)stream;

- (BOOL)userExistsWithJID:(QBXMPPJID *)jid xmppStream:(QBXMPPStream *)stream;

- (void)clearAllResourcesForXMPPStream:(QBXMPPStream *)stream;
- (void)clearAllUsersAndResourcesForXMPPStream:(QBXMPPStream *)stream;
...
**/
#if TARGET_OS_IPHONE
- (void)setPhoto:(UIImage *)image forUserWithJID:(QBXMPPJID *)jid xmppStream:(QBXMPPStream *)stream;
#else
- (void)setPhoto:(NSImage *)image forUserWithJID:(QBXMPPJID *)jid xmppStream:(QBXMPPStream *)stream;
#endif
...
**/
- (void)xmppRoster:(QBXMPPRoster *)sender didReceivePresenceSubscriptionRequest:(QBXMPPPresence *)presence;
