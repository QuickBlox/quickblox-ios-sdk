...
extern NSString *const QBXMPPSRVResolverErrorDomain;


@interface QBXMPPSRVResolver : NSObject
{
	id delegate;
...
@protocol QBXMPPSRVResolverDelegate

- (void)srvResolver:(QBXMPPSRVResolver *)sender didResolveRecords:(NSArray *)records;
- (void)srvResolver:(QBXMPPSRVResolver *)sender didNotResolveDueToError:(NSError *)error;
...
@interface QBXMPPSRVRecord : NSObject
{
	UInt16 priority;
	UInt16 weight;
	UInt16 port;
...
+ (QBXMPPSRVRecord *)recordWithPriority:(UInt16)priority weight:(UInt16)weight port:(UInt16)port target:(NSString *)target;
