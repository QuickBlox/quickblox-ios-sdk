...
struct  {
    static let width: UInt = UInt(UIScreen.main.bounds.width)
    static let height: UInt = UInt(UIScreen.main.bounds.height)
}
...
class SelectedUsersCountAlert: UIViewController {
    ...
    private func setupViews() {
        let topBarHeight = navigationController?.navigationBar.frame.height ?? 44.0
