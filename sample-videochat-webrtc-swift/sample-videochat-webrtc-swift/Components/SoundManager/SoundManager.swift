...
class SoundManager {
    ...
    class func playCallingSound() {
        SoundManager.instance.playSoundWithName(filename: SoundManagerConstant.calling, extention: SoundManagerConstant.wav)
    }
    
    class func playBusySound() {
        SoundManager.instance.playSoundWithName(filename: SoundManagerConstant.busy, extention: SoundManagerConstant.wav)
    }
    
    class func playEndOfCallSound() {
        SoundManager.instance.playSoundWithName(filename: SoundManagerConstant.endOfCall, extention: SoundManagerConstant.wav)
    }
    
    class func playRingtoneSound() {
        SoundManager.instance.playAlertSoundWithName(filename: SoundManagerConstant.ringtone, extention: SoundManagerConstant.wav)
    }
     ...
     */
    func playSoundWithName(filename: String, extention: String?) {
        playSound(filename: filename, ext: extention) {
            debugPrint("playSoundWithName")
        }
    }
    ...
    func playSound(withName filename: String, ext: String?, isAlert: Bool, completion: @escaping () -> Void) {
        ...
        let error = AudioServicesAddSystemSoundCompletion(soundID,
                                                          ...
                                                          {soundID, weakSelfPointer in
                                                            ...
                                                            let weakSelfValue = Unmanaged<SoundManager>.fromOpaque(weakSelfPointer).takeUnretainedValue()
     ...
     */
    func playAlertSoundWithName(filename: String, extention: String?) {
        playAlertSound(filename: filename, ext: extention) {
            debugPrint("playAlertSoundWithName")
        }
    }
    
    func playAlertSound(filename: String, ext: String?, completion: @escaping () -> Void) {
        playSound(withName: filename, ext: ext, isAlert: true, completion: completion)
    }
    
    func playSound(filename: String, ext: String?, completion: @escaping () -> Void) {
    ...
    func systemServicesSoundCompletion(soundID: SystemSoundID, data: UnsafeMutableRawPointer?) {
        let completion: (() -> Void)? = SoundManager.instance.completionBlock(for: soundID)
        if completion != nil {
            completion?()
            SoundManager.instance.removeCompletionBlock(for: soundID)
        }
    }
    ...
    func setOn(_ on: Bool) {
        ...
        userDefaults.set(on, forKey: SoundManagerConstant.soundManagerSettingKey)
