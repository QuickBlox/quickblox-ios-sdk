...
- (void)addID:(NSString *)elementID
        block:(void (^)(id obj, id <QBXMPPTrackingInfo> info))block
...
@protocol QBXMPPTrackingInfo <NSObject>
...
@interface QBXMPPBasicTrackingInfo : NSObject <QBXMPPTrackingInfo>
{
	id target;
	SEL selector;
	
	void (^block)(id obj, id <QBXMPPTrackingInfo> info);
...
- (id)initWithBlock:(void (^)(id obj, id <QBXMPPTrackingInfo> info))block timeout:(NSTimeInterval)timeout;
