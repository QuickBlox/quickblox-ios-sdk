...
- (id)appActiveStateObserver {
    ...
    _appActiveStateObserver = [center addObserverForName:UIApplicationDidBecomeActiveNotification
                                                  object:nil
...
- (id)appInactiveStateObserver {
    ...
    NSNotificationCenter *center = NSNotificationCenter.defaultCenter;
    __weak __typeof(self)weakSelf = self;
    _appInactiveStateObserver = [center addObserverForName:UIApplicationWillResignActiveNotification
                                                  object:nil
                                                   queue:NSOperationQueue.mainQueue
                                              usingBlock:^(NSNotification * _Nonnull note) {
...
- (void)establishConnection {
    ...
    if (QBSession.currentSession.tokenHasExpired) {
        ...
        if (self.onStartAuthorization) { self.onStartAuthorization(); }
        __weak __typeof(self)weakSelf = self;
        [QBRequest logInWithUserLogin:profile.login
            ...
            if (weakSelf.onAuthorize) { weakSelf.onAuthorize(); }
            [weakSelf connectWithId:profile.ID
                           password:profile.password];
        } errorBlock:^(QBResponse * _Nonnull response) {
            weakSelf.isProcessing = NO;
            if (weakSelf.onAuthorizeFailed) { weakSelf.onAuthorizeFailed(); }
        }];
...
- (void)connectWithId:(NSUInteger)id password:(NSString *)password {
    ...
    if (self.onStartConnection) { self.onStartConnection(); }
